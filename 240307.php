...
class lucha {
function $ataqueLuchador() {
    $ataqueLuchador = rand(1,100);
    if ($this->forca == $luchador->forca) {
        if ($ataqueLuchador => 50) {
            $dano = $forcaLuchador1 - $defensaLuchado2;
            return $dano; 
        }
    } elseif ($this->forca => $luchador->forca) {
        if ($ataqueLuchador => 30) {
     ...
     ($ataqueLuchador => 50) {
        ...
        echo "No le ha golpeado";
    }
}

function 
if ($forcaLuchador1 > $forcaLuchador2) {
    if ($ataqueLuchador1 => 30) {
        $dano = $forcaLuchador1 - $defensaLuchado2;
        return $dano;
    } else {
    echo "No le ha golpeado";
